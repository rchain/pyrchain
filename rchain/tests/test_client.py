...
@contextmanager
def deploy_service(deploy_service: Union[DeployServiceServicer, ProposeServiceServicer]) -> Generator[Tuple[grpc.Server, int], None, None]:
...
def test_client_deploy(key: PrivateKey, terms: str, phlo_price: int, phlo_limit: int, valid_after_block_no: int,
    ...
    class DummyDeploySerivce(DeployServiceServicer):
        def doDeploy(self, request: DeployDataProto, context: grpc.ServicerContext):
    ...
    with deploy_service(DummyDeploySerivce()) as (server, port), \
        ...
        assert verify_deploy_data(key.get_public_key(), bytes.fromhex(ret), create_deploy_data(key, terms, phlo_price, phlo_limit, valid_after_block_no, timestamp_millis))

def test_client_show_block():
...
def test_client_show_blocks():
    ...
    faultTolerance = 0.2
    class DummyDeploySerivce(DeployServiceServicer):
        def getBlocks(self, request: BlocksQuery, context: grpc.ServicerContext) -> Generator[BlockInfoResponse, None, None]:
